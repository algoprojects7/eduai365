...
export default function SocialNetworkPage() {
  ...
  const [currentUserName, setCurrentUserName] = useState<string>('Aarav Mehta');
  const [currentUserAvatar, setCurrentUserAvatar] = useState<string>('from-purple-500 to-fuchsia-500');
    ...
    async function loadUserContext() {
      try {
        ...
        if (me) {
          ...
          setCurrentUserAvatar(getAvatarColorForRole(me.role));
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      const schoolKeywords = ['school', 'class', 'library', 'canteen', 'counsellor', 'student', 'teacher', 'club', 'homework', 'exam', 'suggestion', 'issue', 'repair', 'lab', 'ventilator', 'recycling', 'campus', 'thanks', 'great', 'awesome', 'job'];
      const schoolMatches = schoolKeywords.filter(word => text.includes(word)).length;
      // Lower baseline so random unknown text isn't 70%
      const score = Math.min(100, 30 + schoolMatches * 15);
  ...
  const getRoleBadgeColor = (role: string) => {
    switch (role) {
      case 'PRINCIPAL':
      case 'VICE_PRINCIPAL':
        return 'bg-violet-950/40 text-violet-300 border-violet-800/60 shadow-[0_0_12px_rgba(139,92,246,0.15)]';
      case 'COUNSELLOR':
        return 'bg-emerald-950/40 text-emerald-700 border-emerald-800/60 shadow-[0_0_12px_rgba(16,185,129,0.15)]';
      case 'LIBRARIAN':
        return 'bg-amber-950/40 text-amber-300 border-amber-800/60 shadow-[0_0_12px_rgba(245,158,11,0.15)]';
      case 'TEACHER':
        return 'bg-rose-950/40 text-rose-700 border-rose-800/60 shadow-[0_0_12px_rgba(244,63,94,0.15)]';
      case 'PARENT':
        return 'bg-teal-950/40 text-teal-300 border-teal-800/60 shadow-[0_0_12px_rgba(20,184,166,0.15)]';
      default:
        return 'bg-fuchsia-950/40 text-fuchsia-300 border-fuchsia-800/60 shadow-[0_0_12px_rgba(217,70,239,0.15)]';
    }
  };
  ...
  return (
    <StudentShell>
      <div className="space-y-6 max-w-6xl mx-auto min-h-screen text-slate-900 p-1">
        ...
        <header className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-zinc-900 to-indigo-950 border border-slate-200 p-6 md:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          <div className="absolute top-0 right-0 w-80 h-80 bg-indigo-500/10 rounded-full blur-[100px] pointer-events-none" />
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-fuchsia-500/5 rounded-full blur-[120px] pointer-events-none" />
          
          <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-3">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-700 font-semibold text-xs tracking-wider uppercase shadow-inner">
                <Brain className="h-3.5 w-3.5 text-indigo-400 animate-pulse" />
                Ollama gemma4:latest Moderation Engine Active
              </div>
              <h1 className="text-3xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-200 to-indigo-300 tracking-tight">
                School Social Network
              </h1>
              <p className="text-body-md text-slate-600 max-w-2xl leading-relaxed">
                Connect with Greenfield Academy members. Post constructive issues and suggestions. 
                Our local AI engine monitors relevance, ensuring an encouraging, positive environment.
              </p>
            </div>
            <div className="flex flex-col items-start md:items-end gap-3 shrink-0">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3.5 py-1.5 text-xs font-semibold text-emerald-400 border border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.1)]">
                <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
                Online Sync Enabled
              </span>
              <span className="text-xs text-slate-500 font-mono">Host: localhost:11434</span>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-1 flex flex-col gap-6 lg:h-[800px]">
            ...
            {/* Active boards */}
            <div className="bg-slate-100 backdrop-blur-md border border-slate-200 rounded-2xl p-5 shadow-lg space-y-4">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">
                School Channels
              </h3>
              <div className="space-y-1">
                <button className="w-full flex items-center gap-3 px-3 py-2.5 bg-gradient-to-r from-indigo-950/50 to-slate-900/20 border border-indigo-900/40 text-indigo-700 rounded-xl font-semibold text-body-md transition-all">
          ...
          <div className="lg:col-span-3 bg-white shadow-sm backdrop-blur-lg border border-slate-200 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex flex-col h-[800px] lg:h-[800px] overflow-hidden">
            
            {/* Header info */}
            <div className="px-6 py-4 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between">
              <div>
                <h2 className="text-title-md font-bold text-slate-900 flex items-center gap-2">
                  <MessageSquare className="h-5 w-5 text-indigo-400" />
                  # suggestions-issues
                </h2>
                <p className="text-xs text-slate-600 mt-1">
                  General school discussions, queries, and constructive reviews.
                </p>
              </div>
              <div className="text-xs text-indigo-700 bg-indigo-950/40 border border-indigo-800/50 rounded-full px-3 py-1.5 flex items-center gap-1.5 shadow-inner">
                <span className="h-1.5 w-1.5 rounded-full bg-indigo-400 animate-pulse" />
                <span>Text-Only Active</span>
              </div>
            </div>

            {/* Chat message history stream */}
            <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-gradient-to-b from-slate-50 via-slate-50/50 to-slate-50">
              {loadingPosts ? (
            ...
            {/* AI Warning Alerts */}
            {errorMsg && (
              <div className="absolute bottom-[130px] left-0 right-0 z-10 px-6 py-4 bg-rose-50 border-t border-b border-rose-200 flex items-start gap-3.5 text-left text-rose-800 text-body-md shadow-lg animate-in-fade">
                <ShieldAlert className="h-5 w-5 text-rose-500 shrink-0 mt-0.5" />
                <div className="space-y-1">
                  <p className="font-bold text-sm text-rose-700">Message Filtered by Ollama AI</p>
                  <p className="text-xs leading-relaxed text-rose-700">{errorMsg}</p>
                </div>
                <button onClick={() => setErrorMsg(null)} className="ml-auto text-rose-500 hover:text-rose-700 font-bold text-lg leading-none">
                  &times;
                </button>
              </div>
            )}
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 relative">
              ...
              <div className="mb-3 px-3 py-2 min-h-[44px] rounded-xl bg-slate-100 border border-slate-200 flex items-center gap-2 text-left transition-all duration-500">
                <Sparkles className="h-4.5 w-4.5 text-amber-400 shrink-0 animate-bounce" />
                <div className="text-[11px] text-slate-600 leading-tight">
                  <span className="font-bold text-amber-300">Suggestion Helper: </span>
              ...
              <form onSubmit={handleSendMessage} className="space-y-3">
                <div className="flex gap-3 items-stretch">
                  <input
                    ...
                    className="flex-1 rounded-2xl border border-slate-200 bg-white px-5 py-3 text-body-md text-slate-900 placeholder-slate-300 focus:bg-slate-100 focus:border-indigo-500 outline-none shadow-inner transition-all focus:ring-1 focus:ring-indigo-500/25"
                  />
          ...
          <div className="lg:col-span-1 flex flex-col gap-6 lg:h-[800px]">
            ...
            <div className="bg-gradient-to-b from-slate-900/90 to-indigo-950/50 backdrop-blur-md border border-slate-200 rounded-2xl p-5 shadow-lg space-y-5">
              ...
              <div className="space-y-2 text-left">
                <div className="flex justify-between text-xs">
                  ...
                  <span className={`font-mono font-bold ${relevanceScore > 60 ? 'text-indigo-400' : 'text-rose-400'}`}>
                    {relevanceScore}%
                  </span>
                </div>
                <div className="h-2 w-full bg-slate-50 rounded-full overflow-hidden border border-slate-200">
                  <div 
                    className={`h-full rounded-full transition-all duration-300 ${
                      ...
                      safetyStatus === 'empty' ? 'bg-slate-800' : 'bg-rose-600'
                    }`} 
              ...
              <div className="rounded-xl bg-slate-50/80 border border-slate-200 p-3.5 text-left space-y-1">
                <span className="text-xs text-slate-600 font-bold uppercase tracking-wider block">Ollama Evaluation</span>
                {safetyStatus === 'empty' && (
