...
import { clearTokens, isAuthenticated, setTenantSlug, setTokens } from '@/lib/auth';
import type { LoginResult } from '@/types/teacher';

export default function LoginPage() {
  const router = useRouter();
  const [email, setEmail] = useState('teacher@greenfield.eduai365.ai');
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.user.role !== 'TEACHER') {
        clearTokens();
        setError('This portal is for teachers only. Please use the correct role portal.');
        return;
      }
